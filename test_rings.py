...
# count L in [74, 303], found by randomized mirror-DFS with restarts
# (/tmp/palfind.c, 2026-07-23; hardest was L = 301 at 409M nodes).
# Together with PAL_SPECTRUM (exact, nonzero for L = 17..73) and the
# 338/339 witnesses above, palindromic touches exist for every
# L in [17, 303] and at 338, 339; [304, 336] is still open ground.
...
PAL_MID_WITNESSES_BLOB = (
    ...
    "KWGkaqgRuesZrauXFsv3Qh5b/QeAk+vF"
)
PAL_4732 = (
...
def test_palindromic_touch_exists_every_mid_length_grandsire_triples():
    ...
    # per lead count L in [74, 303] and verify each end-to-end. With
    ...
    for line in lines.splitlines():
        ls, half = line.split(":")
        wit[int(ls)] = half
    assert sorted(wit) == list(range(74, 304))
    m = rings.find_method("Grandsire Triples")
